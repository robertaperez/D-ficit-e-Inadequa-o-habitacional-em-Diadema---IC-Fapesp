...
setwd("~/Desktop/IC FAPESP/Codigos")
...
# importa os dados
cadunico <- read_excel("~/Desktop/IC FAPESP/CadÚnico Diadema/cadunico_pessoa+familia.xlsx")

# agrupa por famílias
cadunico_familias <- cadunico[,c(1:38)] %>%
  distinct() %>%
...
cadunico_D07 <- cadunico_familias %>%
 filter(`Codigo familiar` %in% filter(cadunico, `Situacao de Rua` == "Sim")$`Codigo familiar`) %>%
...
cadunico_componentes <- cadunico_familias %>%
  ...
  mutate(D01 = case_when(`Especie do domicilio` == 
         ...
         D02 = case_when(`Material predominante nas paredes eternas do domicilio` 
                         %in% c("taipa nao revestida", "madeira aproveitada", 
                                "palha", "outro material") ~ "Inadequado",
                         `Material predominante nas paredes eternas do domicilio` 
         ...
         # Déficit: famílias em situação de rua (D07)
          D07 = if_else(`Codigo familiar` %in% cadunico_D07$`Codigo familiar`, "Inadequado", "Adequado"),
         ...
         I02 = case_when(`Eistencia de banheiro` == "sim" ~ "Adequado",
                         `Eistencia de banheiro` == "nao" ~ "Inadequado"),
         ...
         I08 = case_when(`Forma de coleta do lio` %in% c("coletado diretamente", "coletado indiretamente") 
                         & `Situacao do domicilio` == "urbana" ~ "Adequado",
                         `Forma de coleta do lio` %in% c("queimado ou enterrado na propriedade", 
...
tabela1 <- cadunico_componentes %>%
  ...
  filter(Codigo %in% c("D01", "D02", "D03", "D04", "D05", "D06", "D07", "Déficit")) %>%
  mutate("%" = Inadequado/6917) ## !!! sempre conferir esse valor
...
tabela2 <- cadunico_componentes %>%
  ...
  mutate(`Sem Dados ou Não Aplicável` = `Sem Dados ou Não Aplicável` + 6917, 
         "%" = Inadequado/22891) ## !!! sempre conferir esse valor
...
write.xlsx(cadunico_componentes, "cadunico_componentes_deficit_inadequacao.xlsx")
